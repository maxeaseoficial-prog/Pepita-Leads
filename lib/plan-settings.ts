...
export const DEFAULT_PLAN_SETTINGS:ManagedPlanSetting[]=[
  {id:"free",name:"Grátis",priceCents:0,searchLimit:3,resultsPerSearch:20,resultLimit:60,popular:false,stripePriceId:""},
  {id:"basic",name:"Basic",priceCents:2990,searchLimit:30,resultsPerSearch:20,resultLimit:600,popular:true,stripePriceId:""},
  {id:"unlimited",name:"Unlimited",priceCents:9990,searchLimit:null,resultsPerSearch:20,resultLimit:null,popular:false,stripePriceId:""}
];
...
export async function loadPlanSettings():Promise<ManagedPlanSetting[]> {
  try {
    ...
    const rows=await sql.query(
      "select plan_id,display_name,price_cents,search_limit,results_per_search,result_limit,is_popular,stripe_price_id from plan_settings order by case plan_id when 'free' then 1 when 'basic' then 2 else 3 end"
    ) as unknown as Row[];
  ...
  for(const plan of plans) {
    ...
    await sql.query(`
      insert into plan_settings(
        plan_id,display_name,price_cents,search_limit,results_per_search,result_limit,is_popular,stripe_price_id,updated_at
